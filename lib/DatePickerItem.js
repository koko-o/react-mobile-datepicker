...
class DatePickerItem extends Component<void, Props, State> {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if(this.typeName === 'list') return true;
    ...
    _moveToNext(direction) {
        if(this.typeName === 'list'){
            if (direction === -1 && this.currentIndex < 0) {
                this._updateDates(1);
            } else if (direction === 1 && this.currentIndex > this.state.dates.length-1) {
                this._updateDates(-1);
            }
        } else {
    ...
    _moveTo(obj, currentIndex) {
        ...
        setTimeout(() => {
            ...
            this.props.onSelect(this.typeName === 'list' ? this.state.dates[currentIndex] : this.state.dates[MIDDLE_INDEX]);
    ...
    handleMove(event) {
        ...
        if(this.typeName === 'list'){
            if (this.currentIndex < 0 || this.currentIndex > this.state.dates.length-1){
                return;
            }
        }else {
            // 日期最小值，最大值限制
